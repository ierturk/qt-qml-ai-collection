...
size_t OrtUtils::vectorProduct(const std::vector<int64_t>& vector)
{
    if (vector.empty())
        return 0;
...
std::wstring OrtUtils::charToWstring(const char* str)
{
...
std::vector<std::string> OrtUtils::loadNames(const std::string& path)
{
    // load class names
...
void OrtUtils::visualizeDetection(cv::Mat& image, std::vector<Detection>& detections,
...
void OrtUtils::letterbox(const cv::Mat& image, cv::Mat& outImage,
...
void OrtUtils::scaleCoords(const cv::Size& imageShape, cv::Rect& coords, const cv::Size& imageOriginalShape)
{
...
template <typename T>
T OrtUtils::clip(const T& n, const T& lower, const T& upper)
{
